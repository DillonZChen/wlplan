...
std::shared_ptr<feature_generator::Features> load_feature_generator(const std::string save_file) {
  ...
  std::shared_ptr<feature_generator::Features> feature_generator;
  if (feature_name == "wl") {
    feature_generator = std::make_shared<feature_generator::WLFeatures>(save_file);
  } else if (feature_name == "2-lwl") {
    feature_generator = std::make_shared<feature_generator::LWL2Features>(save_file);
  } else if (feature_name == "ccwl") {
    feature_generator = std::make_shared<feature_generator::CCWLFeatures>(save_file);
  } else if (feature_name == "ccwl-a") {
    feature_generator = std::make_shared<feature_generator::CCWLaFeatures>(save_file);
  } else if (feature_name == "iwl") {
    feature_generator = std::make_shared<feature_generator::IWLFeatures>(save_file);
  } else if (feature_name == "niwl") {
    feature_generator = std::make_shared<feature_generator::NIWLFeatures>(save_file);
  } else {
